...
impl Transcoder {
    ...
    pub unsafe fn prepare(&mut self) -> Result<DemuxerInfo> {
        unsafe { self.demuxer.probe_input() }
    }
    ...
    fn next(&mut self) -> Result<bool> {
        let (pkt, stream) = unsafe { self.demuxer.get_packet()? };
        ...
        let src_index = unsafe { (*stream).index };
        // check if encoded stream
        if let Some(enc) = self.encoders.get_mut(&src_index) {
            for (frame, _stream) in self.decoder.decode_pkt(Some(&pkt))? {
            ...
            // Need to clone and modify stream_index for copy streams
            let pkt_clone = pkt.clone();
            unsafe { (*pkt_clone.ptr()).stream_index = *dst_stream };
            self.muxer.write_packet(&pkt_clone)?;
        }
